...
function openReading(id) {
  ...
  if (!window.location.pathname.includes('reading.html')) {
    // 记录主界面滚动位置，从阅读页返回时恢复（避免回到顶部）
    try { sessionStorage.setItem('yanshu_reading_return_y', String(window.scrollY || 0)); } catch (e) { /* 忽略 */ }
 ...
 */
/** 结构化研读内容渲染：中文为主，英文原文可折叠对照（含页码）。无数据返回空串。 */
function renderStructuredZhEn(paper, key, enLabel) {
  const st = paper && paper.structured;
  if (!st) return '';
  const zh = st.zh && st.zh[key];
  const en = st[key];
  let html = '';
  if (zh && String(zh).trim()) {
    html += '<p class="reading-lead" style="white-space:pre-line;">' + escapeHtml(String(zh)) + '</p>';
  }
  if (en && en.content) {
    const pages = Array.from(new Set((en.evidence || []).map(e => e.page).filter(p => p != null)));
    const pageText = pages.length ? '（原文第 ' + pages.join('、') + ' 页）' : '';
    const paras = String(en.content).split('\n\n').map(p => '<p>' + escapeHtml(p) + '</p>').join('');
    html += '<details class="structured-en" style="margin-top:12px;background:#F8FAFC;border:1px solid #E5E7EB;border-radius:8px;padding:10px 12px;">'
      + '<summary style="cursor:pointer;font-size:12px;font-weight:600;color:var(--brand-blue);">' + enLabel + pageText + '</summary>'
      + '<div style="font-size:12px;color:var(--text-mid);line-height:1.7;margin-top:8px;">' + paras + '</div>'
      + '</details>';
  }
  return html;
}

function buildReadingTabContent(paper, tab) {
  const keywords = paper.keywords || [];
  const chips = keywords.map(k => '<span class="reading-chip">' + escapeHtml(k) + '</span>').join('');
  const title = escapeHtml(paper.title || '当前论文');
  const field = keywords.length ? escapeHtml(keywords.slice(0, 3).join(' / ')) : '相关研究方向';
  const zhSummary = paper.structured && paper.structured.zh && paper.structured.zh.summary;
  const baseInfo = '<div class="reading-insight-grid">'
    + '<div class="reading-insight-card"><span class="reading-insight-label">发表</span><strong>' + escapeHtml(paper.venue || '未知') + '</strong></div>'
    + '<div class="reading-insight-card"><span class="reading-insight-label">年份</span><strong>' + escapeHtml((paper.year || 'N/A').toString()) + '</strong></div>'
    + '<div class="reading-insight-card"><span class="reading-insight-label">引用</span><strong>' + escapeHtml(paper.citations || 'N/A') + '</strong></div>'
    + '<div class="reading-insight-card"><span class="reading-insight-label">CCF</span><strong>' + escapeHtml(paper.ccf || 'N/A') + '</strong></div>'
    + '</div>';
  const contents = {
    summary: '<div class="reading-ai-section"><h4>AI 速读</h4>'
      + (zhSummary
          ? '<p class="reading-lead" style="white-space:pre-line;">' + escapeHtml(zhSummary) + '</p>'
            + (paper.abstract ? '<details class="structured-en" style="margin-top:10px;background:#F8FAFC;border:1px solid #E5E7EB;border-radius:8px;padding:10px 12px;"><summary style="cursor:pointer;font-size:12px;font-weight:600;color:var(--brand-blue);">📄 英文摘要</summary><p style="font-size:12px;color:var(--text-mid);line-height:1.7;margin-top:8px;white-space:pre-line;">' + escapeHtml(paper.abstract) + '</p></details>' : '')
          : '<p class="reading-lead">' + escapeHtml(paper.abstract || '暂无摘要') + '</p>')
      + (chips ? '<div class="reading-chip-row">' + chips + '</div>' : '')
      + baseInfo
      + '<div class="reading-note-box"><strong>研读建议</strong><p>先定位论文解决的问题，再对照摘要中的方法词和实验指标，判断其贡献是架构、训练策略、数据构造还是应用验证。</p></div></div>',
    novelty: (paper.structured && (paper.structured.zh?.core_innovation || paper.structured.core_innovation?.content))
      ? '<div class="reading-ai-section"><h4>核心创新</h4>' + renderStructuredZhEn(paper, 'core_innovation', '📄 英文原文') + '</div>'
      : '<div class="reading-ai-section"><h4>创新点拆解</h4><div class="reading-checklist">'
        + '<div><strong>问题重述</strong><span>论文围绕 ' + field + ' 展开，可从任务痛点和已有方法限制切入。</span></div>'
        + '<div><strong>方法贡献</strong><span>关注作者是否提出新结构、新训练目标、新数据流程或更强的组合策略。</span></div>'
        + '<div><strong>影响范围</strong><span>结合引用量、发表 venue 和关键词，判断该工作适合作为方法基线还是背景综述。</span></div>'
        + '</div></div>',
    experiment: (paper.structured && (paper.structured.zh?.experiments || paper.structured.experiments?.content))
      ? '<div class="reading-ai-section"><h4>实验与结果</h4>' + renderStructuredZhEn(paper, 'experiments', '📄 英文原文') + '</div>'
      : '<div class="reading-ai-section"><h4>实验阅读清单</h4><div class="reading-checklist">'
        + '<div><strong>数据集</strong><span>记录主实验数据集、规模、划分方式和是否公开。</span></div>'
        + '<div><strong>基线</strong><span>检查是否覆盖强基线、同年份方法和消融版本。</span></div>'
        + '<div><strong>指标</strong><span>关注主指标提升是否稳定，是否报告方差、置信区间或多次运行。</span></div>'
        + '<div><strong>消融</strong><span>验证每个模块是否单独带来收益，避免只看最终分数。</span></div>'
        + '</div></div>',
    qa: '<div class="reading-ai-section"><h4>论文问答</h4>'
      + '<p class="reading-lead">围绕「' + title + '」继续追问，AI 会带入标题、作者、venue、摘要和关键词作为上下文。</p>'
      + '<div class="reading-question-grid">'
      + '<button onclick="fillReadingQuestion(\'这篇论文的核心贡献是什么？\')">核心贡献</button>'
      + '<button onclick="fillReadingQuestion(\'这篇论文的实验设计是否充分？\')">实验充分性</button>'
      + '<button onclick="fillReadingQuestion(\'这篇论文适合和哪些工作对比？\')">相关工作对比</button>'
      + '<button onclick="fillReadingQuestion(\'这篇论文有哪些局限和可改进方向？\')">局限分析</button>'
      + '</div></div>',
    translate: '<div class="reading-ai-section"><h4>翻译与术语</h4>'
      + '<p class="reading-lead">选中左侧正文后可使用浮动菜单翻译。当前标题可先作为术语对齐入口：</p>'
      + '<div class="reading-note-box"><strong>标题</strong><p>' + title + '</p></div></div>'
  };
  return contents[tab] || contents.summary;
}

function fillReadingQuestion(question) {
...
function switchReadingAiTab(el, tab) {
  ...
  const contents = {
    summary: '<div class="ai-reading-summary">'
      ...
      + (paper.structured && paper.structured.zh && paper.structured.zh.summary
          ? '<p class="zh"><strong>论文摘要（中文研读）</strong></p>'
            + '<p class="zh" style="white-space:pre-line;">' + escapeHtml(paper.structured.zh.summary) + '</p>'
            + (paper.abstract ? '<p class="zh" style="margin-top:10px;"><strong>英文摘要</strong></p>'
              + '<details class="structured-en" style="margin-top:6px;background:#F8FAFC;border:1px solid #E5E7EB;border-radius:8px;padding:10px 12px;"><summary style="cursor:pointer;font-size:12px;font-weight:600;color:var(--brand-blue);">📄 英文摘要</summary><p style="font-size:12px;color:var(--text-mid);line-height:1.7;margin-top:8px;white-space:pre-line;">' + escapeHtml(paper.abstract) + '</p></details>' : '')
      ...
      + '</div>',
    novelty: (paper.structured && (paper.structured.zh?.core_innovation || paper.structured.core_innovation?.content))
      ? '<h4 style="font-size:14px;font-weight:600;margin-bottom:8px;color:var(--text-dark);">核心创新</h4>' + renderStructuredZhEn(paper, 'core_innovation', '📄 英文原文')
      : '<h4 style="font-size:14px;font-weight:600;margin-bottom:8px;color:var(--text-dark);">创新点分析</h4>'
    ...
    experiment: (paper.structured && (paper.structured.zh?.experiments || paper.structured.experiments?.content))
      ? '<h4 style="font-size:14px;font-weight:600;margin-bottom:8px;color:var(--text-dark);">实验与结果</h4>' + renderStructuredZhEn(paper, 'experiments', '📄 英文原文')
      : '<h4 style="font-size:14px;font-weight:600;margin-bottom:8px;color:var(--text-dark);">实验分析</h4>'
  ...
  if (tab === 'similar') {
    ...
    contentDiv.innerHTML = buildReadingTabContent(paper, tab);
  }
}// ===== 公式解释数据 =====
const formulaData = {
  attention: {
...
function exportCitation(paperId, format) {
  const localFallback = (typeof fallbackPapers !== 'undefined' ? fallbackPapers : (window.papers || []));
  const paper = (window.papers || papers || []).find(p => p.id === paperId) || localFallback.find(p => p.id === paperId);
  ...
  (async function() {
    try {
      const msgs = [
        ...
        { role: 'user', content: '我正在精读一篇论文，请基于论文信息给出结构化研读成果。\n论文ID：' + (paperId || '') + '\n标题：' + (paper?.title || '未知') + '\n作者：' + (paper?.authors || '未知') + '\n发表：' + (paper?.venue || '未知') + '\n摘要：' + (paper?.abstract || '无') + '\n\n我的问题是：' + question }
      ];
      const reply = await deepseekChat(msgs, false, { taskType: 'ai_reading' });
      const ans = document.getElementById('readingQaAnswer');
      if (ans) ans.innerHTML = '<strong>AI答：</strong><div class="reading-ai-answer">' + renderMdInline(reply) + '</div>';
      // 添加到历史记录
      ...
      if (hist) hist.innerHTML = '<div style="border-top:1px solid #E5E7EB;margin-top:10px;padding-top:10px;">'
        ...
        + '<div style="background:#F8FAFC;padding:8px 12px;border-radius:8px;font-size:12px;border:1px solid #E5E7EB;"><strong>A:</strong><div class="reading-ai-answer">' + renderMdInline(reply) + '</div></div>'
        + '</div>' + hist.innerHTML;
    } catch(e) {
      const ans = document.getElementById('readingQaAnswer');
