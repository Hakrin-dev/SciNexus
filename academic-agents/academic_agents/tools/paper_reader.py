...
def extract_url_and_question(user_input: str) -> tuple[str, str]:
    ...
    if not match:
        raise ValueError(
            "论文精读需要一个 http/https 论文链接。建议提供可直接打开的 PDF 链接。"
        )
...
def _request_with_fallback(
    ...
    for _ in range(6):
        ...
        for connection_mode in ("system", "direct", "tls12"):
            try:
                candidate = _request_once(
                    current_url, headers, timeout, connection_mode
                )
    ...
    raise RuntimeError(
        "无法下载论文（已尝试系统连接、直连和 TLS 1.2）："
        f"{diagnostics}。请尝试复制浏览器中的直接 PDF 下载链接。"
    ) from last_error
...
def download_pdf(url: str, timeout: float = 12.0) -> tuple[bytes, str]:
    ...
    headers = {
        ...
        "User-Agent": "AcademicAgentsStarter/0.1",
    }
...
def extract_pdf_chunks(pdf_bytes: bytes, chunk_size: int = 1800) -> tuple[list[PaperChunk], int]:
    ...
    for page_number, page in enumerate(reader.pages, start=1):
        ...
        while start < len(text):
            ...
            chunks.append(
                PaperChunk(
                    page=page_number,
                    chunk_id=f"p{page_number}-c{part}",
                    text=chunk_text,
                )
            )
    ...
    if not chunks:
        raise RuntimeError(
            "PDF 没有可提取的文字，可能是扫描图片版；当前版本尚未接入 OCR。"
        )
